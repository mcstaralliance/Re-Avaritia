...
import committee.nova.mods.avaritia.common.container.slot.DataSlots;
import committee.nova.mods.avaritia.common.container.slot.SlotData;
import committee.nova.mods.avaritia.common.container.slot.SlotStorage;
import committee.nova.mods.avaritia.common.sync.TerminalSyncManager;
 ...
 */
public class WipChestMenu extends AbstractContainerMenu implements IDataReceiver, TerminalSyncManager.InteractHandler {
    protected WipChestTile te;
    ...
    public WipChestMenu(int id, Inventory inv, WipChestTile te) {
        this(ModMenus.infinity_chest.get(), id, inv, te);
        this.addPlayerSlots(inv, 8, 120);
    }

    public WipChestMenu(MenuType<?> type, int id, Inventory inv, WipChestTile te) {
        super(type, id);
        this.te = te;
        this.pinv = inv;
        addStorageSlots();
    ...
    public WipChestMenu(MenuType<?> type, int id, Inventory inv) {
        this(type, id, inv, null);
    }

    public WipChestMenu(int id, Inventory inv, FriendlyByteBuf buf) {
        this(ModMenus.infinity_chest.get(), id, inv);
        this.addPlayerSlots(inv, 8, 120);
    }
    ...
    @Override
    protected @NotNull Slot addSlot(@NotNull Slot slotIn) {
    ...
    @Override
    public final @NotNull ItemStack quickMoveStack(@NotNull Player playerIn, int index) {
        if (slots.size() > index) {
